...
const KanbanBoard = () => {
  ...
  const groupTickets = (tickets, groupBy, users) => {
    ...
    if (groupBy === "status") {
      ...
      groupedTickets = tickets.reduce((acc, ticket) => {
        const priorityLabel =
          priorityLabels[ticket.priority] || "Unknown Priority";
  ...
  return (
    <div className="kanban-board">
      ...
      {Object.entries(
        groupTickets(sortTickets(tickets, sortBy), groupBy, users)
      ).map(([group, groupTickets]) => (
        <div key={group} className="group">
          <h2>
            {groupBy === "status" && icons[group.toLowerCase()]}{" "}
            {/* Display status icon */}
            {groupBy === "priority" && icons[group.toLowerCase()]}{" "}
            {/* Display priority icon */}
            {group}
            <span
              className="column-card-count"
              style={{
                fontSize: "15px",
                marginLeft: "5px",
                marginRight: "30px",
            ...
            >
              {groupTickets.length}{" "}
              {/* Correctly accessing the count of cards */}
            </span>
